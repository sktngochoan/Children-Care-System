...
public class CommentSubmit extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (session != null) {
                ac = (user) session.getAttribute("account");
                String name = ac.getUser_name();
                String contentComment = request.getParameter("contentComment");
                commentDao.insetContentComment(name, contentComment);
            } else {
                request.setAttribute("messe", "You must login account!");
