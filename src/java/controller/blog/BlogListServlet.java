...
public class BlogListServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        for(int i = 0;i<listCate.size();i++){
            int quantity = blogsDAO.getQuantityByCateId(listCate.get(i).getCategory_id());
            listQuantity.add(quantity);
        }
        
       request.setAttribute("listCate", listCate);
       List<blog> listBlog = blogsDAO.getAllBlogs();
       ...
       if(request.getParameter("cid")!=null){
          ...
          listBlog = blogsDAO.getAllBlogsByCateId(cid);
          request.setAttribute("listBlog", listBlog);
       }
       if(request.getParameter("txt")!= null){
          String txt = request.getParameter("txt");
          listBlog = blogsDAO.getAllBlogsSearch(txt);
          request.setAttribute("listBlog", listBlog);
       }
       
       request.setAttribute("listQuantity", listQuantity);
       List<blog> listTop5 = blogsDAO.getTop5NewBlogs();
